...
#include <arpa/inet.h> // different address structures are declaredhere
...
int main()
{
    ...
    float *numb,*numbback;
    scanf("%f",numb);
    int bytesSent = send (sock, numb, sizeof(float), 0);
    printf("Message sent: %f  Bytessent = %d\n",*numb,bytesSent);
    ...
    int bytesRecvd = recv (sock, numbback, sizeof(float), 0);
    if (bytesRecvd < 0)
    { 
